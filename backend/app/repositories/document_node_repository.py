...
class DocumentNodeRepository:
    ...
    def __init__(self, db, owner_id: str | None = None):
        self.db = db
        self.owner_id = owner_id

    def _owner(self) -> str:
        if not self.owner_id:
            raise ValueError("DocumentNodeRepository requires an owner_id.")
    ...
    def get_children(self,parent_id: str):
        ...
        FROM document_nodes n
        JOIN documents d ON d.id = n.document_id
        WHERE n.parent_id = %s AND d.user_id = %s
        ORDER BY sequence_no
        """

        return self.db.fetch_all(query,(parent_id, self._owner()))
    ...
    def delete(
        ...
        self.db.execute(query,(node_id, self._owner()))
    ...
    def delete_by_document(
        ...
        query = """
        DELETE FROM document_nodes
        WHERE document_id = %s AND EXISTS (
            SELECT 1 FROM documents d
            WHERE d.id = document_nodes.document_id AND d.user_id = %s
        )
        """
        self.db.execute(query,(document_id, self._owner()))
